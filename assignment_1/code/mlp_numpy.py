...
class MLP(object):
  ...
  def __init__(self, n_inputs, n_hidden, n_classes):
    ...
    self.biases.append(bias_arr)
    weight_arr=np.random.randn(self.n_hidden[self.hidden_layers-1],n_classes[0])
    self.weights.append(weight_arr)
  ...
  def forward(self, x):
    ...
    epsilion=0.05
    mean_h_in = np.mean(h_out, axis=0)
    std_h_in = np.std(h_out, axis=0)
    std_h_in = np.multiply(std_h_in, std_h_in)
    std_h_in+=epsilion
    for t in range(0, 100):
        h_out[:, t] -= mean_h_in[t]
        h_out[:, t] = h_out[:, t] / std_h_in[t]
    h_out=np.multiply(h_out,self.gamma_arr)
    h_out=np.add(h_out,self.beta_arr)
    h_out=np.dot(h_out,self.weights[self.hidden_layers])
    h_out+=self.biases[self.hidden_layers]
    dict_layer["out"]=h_out
    dict_layer["outstd"]=std_h_in
  ...
  def backward(self, dout,batch_size):
    ...
    while(layer_index>0):
        ...
        ##delta_last_layer = np.multiply(delta_last_layer, self.gamma_arr/dout["outstd"])
  ...
  def sgd(self,delta_w,delta_b,lr,delta_gamma, delta_beta):
      delta_index=0
      layer_index=self.hidden_layers
